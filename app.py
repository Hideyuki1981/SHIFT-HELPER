...
　　シート構成 
    `staff`: スタッフ設定（必須）
    `ng_pair`: 禁止ペア設定（任意・英語のまま）

### 2. staffシートの説明
| カラム名 | 説明 |
| :--- | :--- |
| **グループ** | AまたはB、東北、南西などユニット名 |
| **名前** | スタッフの名前 (必須) |
| **属性** | `常勤` `非常勤` `派遣` |
|   |常勤>非常勤>派遣で優先組成 |     
| **公休数** | 月の公休日数 |
...
| **最大連勤** | 最大連勤数 (空欄=6連勤まで) |
| **前月夜** | 前月末が夜勤なら`1`月初が明になる |
| **前月連勤** | 前月末の連勤数を引き継いで組成 |
...
### 3. ng_pairシート（禁止ペア）
相性の悪いペアなどを入力します。グループ内で（早/日/遅）夜勤を避けます。
※「早番」と「夜勤」のような時間帯が被らない組み合わせはOKとみなされます。
協力ユニットを跨いだ設定の場合は夜勤のみに判定されます。
禁止ペアは努力義務でシフトの組成が優先されます。
...
### 4. 再計算用乱数
数字を増やしていくと同条件の別パターンのシフトがランダムに組成されます。
"""
...
with st.sidebar:
    ...
    # 行ラベルの並び順を変更: Aグループ(早日遅夜) → Bグループ(早日遅夜)
    req_index = [
        "早A", "日A", "遅A", "夜A",
        "早B", "日B", "遅B", "夜B"
    ]
